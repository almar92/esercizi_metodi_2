...
Lo sviluppo in serie può essere esteso anche al caso in cui la funzione oggetto di studio $f(z)$ non sia analitica in $z_0$, trovando uno sviluppo che approssima "bene" il comportamento della funzione, ovvero che valgono le due seguenti proprietà:
\begin{enumerate}
	\item il resto della serie è tale che $\limit{z}{z_0} R_N(z) = 0 $
	\item La serie fornisce una buona approssimazione di $f(z)$ solo in dterminate regioni del piano complesso
\end{enumerate}

\subsection{Metodo risolutivo generalizzato (Paolo Proia)}

Dato una funzione integrale
\begin{align}
